...
function CommentBody({ body }: { body: string }) {
  ...
  return (
    <p className="text-[13px] text-white/60 leading-relaxed whitespace-pre-wrap break-words mt-0.5 sm:text-[14px]">
      {shown}
      {long && (
        <button
...
function CommentInput({
  ...
  return (
    <div className="flex items-start gap-2 sm:gap-3">
      ...
      <div className="flex-1 rounded-xl bg-black/25 px-3 py-2.5 ring-1 ring-white/[0.06] transition-colors focus-within:bg-black/35 focus-within:ring-white/[0.12] sm:rounded-2xl sm:px-4 sm:py-3">
        <textarea
          ref={ref}
          value={body}
          ...
          className="min-h-[60px] w-full resize-none bg-transparent text-xs text-white/75 placeholder-white/25 outline-none sm:min-h-[76px] sm:text-sm"
        />
        <div className="mt-1.5 flex items-center justify-end gap-2 sm:mt-2">
          {onCancel && (
            <button onClick={onCancel} className="text-[10px] text-white/30 hover:text-white/60 transition-colors px-2 py-1 sm:text-xs">
              Cancel
            </button>
          )}
          <button
            ...
            className="flex items-center gap-1 rounded-full bg-white px-3 py-1.5 text-[10px] font-bold text-black transition-all hover:bg-white/85 disabled:cursor-not-allowed disabled:opacity-25 sm:gap-1.5 sm:px-3.5 sm:py-2 sm:text-xs"
          >
...
function CommentItem({
  ...
  return (
    ...
    >
      <div className={cn("flex gap-2 py-3 sm:gap-3 sm:py-4", depth > 0 && "ml-3 border-l border-white/[0.06] pl-3 sm:ml-5 sm:pl-4")}>
        ...
        <div className="flex-1 min-w-0 group">
          ...
          <div className="flex items-center gap-1 flex-wrap sm:gap-1.5">
            <span className="text-xs font-semibold text-white/85 sm:text-[13px]">{name}</span>
          ...
          {/* Actions row */}
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-2 sm:gap-4 sm:mt-2.5">
            {/* Upvote (static display) */}
            <button className="flex items-center gap-1 text-[11px] text-white/30 hover:text-white/60 transition-colors group/up sm:gap-1.5 sm:text-[12px]">
              <ThumbsUp className="h-3 w-3 group-hover/up:text-[#E8431C]/70 transition-colors sm:h-3.5 sm:w-3.5" />
              <span>Upvote</span>
            </button>
            ...
            {depth === 0 && (
              <button
                ...
                className="text-[11px] text-white/30 hover:text-white/60 transition-colors sm:text-[12px]"
              >
                Reply
              </button>
            )}
            ...
            {isOwn && (
              <button
                ...
                className="text-[11px] text-red-400/35 hover:text-red-400/70 transition-colors sm:text-[12px] sm:opacity-0 sm:group-hover:opacity-100"
              >
                Delete
              </button>
            )}
            ...
            {/* Timestamp */}
            <span className="text-[10px] text-white/25 sm:ml-auto sm:text-[11px]">{timeAgo(comment.createdAt)}</span>
          </div>
...
export default function CommentsSection({ postId, platform }: { postId: string; platform?: "reddit" | "x" }) {
  ...
  return (
    <section className="rounded-[20px] bg-white/[0.035] p-3 shadow-2xl shadow-black/20 ring-1 ring-white/[0.07] backdrop-blur-xl sm:rounded-[28px] sm:p-5">
      {/* Section header */}
      <div className="mb-3 flex items-start justify-between gap-2 sm:mb-5 sm:items-center sm:gap-3">
        <div>
          <p className="text-[9px] font-bold uppercase tracking-[0.14em] text-white/30 sm:text-[10px] sm:tracking-[0.18em]">Community</p>
          <h3 className="mt-0.5 text-base font-bold text-white sm:mt-1 sm:text-lg">
            {loading ? "Comments" : `${totalCount} Comment${totalCount !== 1 ? "s" : ""}`}
          </h3>
        </div>
        <span className="hidden rounded-full bg-white/[0.05] px-2.5 py-1 text-[10px] text-white/35 sm:inline sm:px-3 sm:text-xs">
          Discuss this market
        </span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="mb-4 flex flex-col gap-3 rounded-2xl bg-black/25 px-3 py-3 ring-1 ring-white/[0.06] sm:mb-5 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:px-4">
          <div className="min-w-0">
            <p className="text-xs font-medium text-white/55 sm:text-sm">
              {isAuthenticated
                ? `Link your ${provider} account to comment`
                : `Sign in with ${provider} to comment`}
            </p>
            {isAuthenticated && (
              <p className="mt-1 text-[10px] leading-relaxed text-white/25 sm:mt-0.5 sm:text-xs">
          ...
          <button
            type="button"
            onClick={() => startProviderSignIn(authProvider)}
            className="w-full shrink-0 rounded-full bg-white px-3 py-2 text-[11px] font-bold text-black transition-colors hover:bg-white/85 sm:w-auto sm:px-3.5 sm:text-xs"
          >
